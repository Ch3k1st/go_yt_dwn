...
- **Расширение браузера** — ловит видео там, где обычная ссылка отвечает «403»: берёт адрес плеера вместе с куками и заголовками. Ставится кнопкой из программы, инструкция — на экране.
...
Раздел «Расширение» показывает найденные на компьютере браузеры и ставит
расширение в один клик: программа распаковывает папку с ключом доступа, открывает
страницу расширений браузера и саму папку, а на экране остаётся пронумерованная
инструкция с путём и адресом, которые можно скопировать. Наверху — индикатор
«Подключено / Не подключено»: он оживает, как только расширение отзовётся.

![Раздел «Расширение»](docs/screenshots/ext-ui-light.png)

В попапе расширения у каждой находки есть маленькое превью и, если поток
многовариантный, выбор качества прямо в строке — открывать программу ради этого
не нужно. Превью берётся сначала со страницы (`og:image`, `poster` у `<video>`),
а если там пусто — кадром из потока, который снимает сама программа.

![Попап расширения](docs/screenshots/ext-popup-light.png)

Пойманные расширением загрузки идут отдельной секцией в разделе «Очередь» —
их ведёт сама программа, поэтому список не пропадает при перезагрузке окна.
Подробности и границы (DRM не поддерживается) — в [docs/extension.md](docs/extension.md).
